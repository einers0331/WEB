var usuarios = '[{"id": 1,"name": "Leanne Graham","username": "Bret","email": "Sincere@april.biz","address": {"street": "Kulas Light", "suite": "Apt. 556","city": "Gwenborough", "zipcode": "92998-3874", "geo": { "lat": "-37.3159", "lng": "81.1496"}}, "phone": "1-770-736-8031 x56442", "website": "hildegard.org", "company": { "name": "Romaguera-Crona", "catchPhrase": "Multi-layered client-server neural-net", "bs": "harness real-time e-markets" }},'+
...
var response = $.parseJSON(usuarios);
var $table = $('<table></table>');
$table.append("<thead><tr><th>ID</th><th>Name</th><th>Username</th><th>Email</th><th>Addres</th><th>Suite</th><th>City</th><th>Zipcode</th><th>Latitude</th><th>Longitude</th><th>Phone</th><th>Website</th><th>Company</th><th>CatchPhrase</th><th>Bs</th></tr></thead>")
	$.each(response, function(i, item) {

		var tr = $('<tr>').append(
			$('<td class="col1">').text(item.id),
			$('<td class="col2">').text(item.name),
			$('<td>').text(item.username),
			$('<td>').text(item.email),
			$('<td>').text(item.address.street),
			$('<td>').text(item.address.suite),
			$('<td>').text(item.address.city),
			$('<td>').text(item.address.zipcode),
			$('<td>').text(item.address.geo.lat),
			$('<td>').text(item.address.geo.lng),
			$('<td>').text(item.phone),
			$('<td>').text(item.website),
			$('<td>').text(item.company.name),
			$('<td>').text(item.company.catchPhrase),
			$('<td>').text(item.company.bs)
  	);
...
$(document).ready(function(){
        ...
        _this = this;      
            ...
            $.each($(".wrapper tbody tr"), function() {
